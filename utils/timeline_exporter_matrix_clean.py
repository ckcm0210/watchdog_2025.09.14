...
def generate_html(events=None):
    ...
    try:
        ...
        html.append('<h2 style="color:#00ff41;text-shadow:0 0 10px #00ff41;">Excel Timeline - Matrix v4</h2>')
        ...
        try:
            # 確保目錄存在
            os.makedirs(os.path.dirname(TIMELINE_HTML_MATRIX_CLEAN), exist_ok=True)
            # 寫入檔案
            with open(TIMELINE_HTML_MATRIX_CLEAN, 'w', encoding='utf-8') as f:
                f.write('\n'.join(html))
            ...
            # 輸出成功訊息
            # 總是顯示成功訊息
            print(f"[timeline-matrix-clean] HTML 成功寫入: {TIMELINE_HTML_MATRIX_CLEAN}")
            if getattr(settings, 'SHOW_DEBUG_MESSAGES', False):
                print(f"[timeline-matrix-clean] HTML 詳細信息已寫入 {TIMELINE_HTML_MATRIX_CLEAN}")
